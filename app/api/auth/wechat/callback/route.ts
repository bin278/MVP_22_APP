...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    console.log('[WeChat Callback] Received code from native SDK');
    ...
    if (!appId || !appSecret) {
      console.error("[WeChat Callback] Missing WECHAT_APP_ID or WECHAT_APP_SECRET");
      return NextResponse.json(
    ...
    console.log(`[WeChat Callback] Exchanging code for token...`);

    const tokenResponse = await fetch(tokenUrl);
    const tokenData = await tokenResponse.json();

    if (tokenData.errcode) {
      console.error('[WeChat Callback] Token exchange failed:', tokenData);
      return NextResponse.json(
    ...
    if (!access_token || !openid) {
      console.error('[WeChat Callback] Invalid token response:', tokenData);
      return NextResponse.json(
    ...
    console.log(`[WeChat Callback] Fetching user info for openid: ${openid}`);

    const userInfoResponse = await fetch(userInfoUrl);
    const userInfo = await userInfoResponse.json();

    if (userInfo.errcode) {
      console.error('[WeChat Callback] User info fetch failed:', userInfo);
      return NextResponse.json(
    ...
    console.log(`[WeChat Callback] User info retrieved: ${nickname}, avatar: ${avatar ? 'yes' : 'no'}`);

    // 3. 调用 CloudBase 认证服务
    console.log(`[WeChat Callback] Calling CloudBase sign in...`);
    ...
    if (!result.success) {
      console.error('[WeChat Callback] CloudBase sign in failed:', result.error);
      return NextResponse.json(
    ...
    console.log(`[WeChat Callback] Authentication successful for user: ${result.user.email}`);
    ...
    console.error('[WeChat Callback] Unexpected error:', error);
    return NextResponse.json(
