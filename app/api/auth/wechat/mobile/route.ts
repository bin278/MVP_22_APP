...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    // 3. 生成微信移动端 OAuth URL (使用 snsapi_userinfo 获取用户信息)
    // 注意：移动端使用 oauth2/authorize 而不是 qrconnect
    const wechatAuthUrl = `https://open.weixin.qq.com/connect/oauth2/authorize?appid=${appId}&redirect_uri=${redirectUri}&response_type=code&scope=snsapi_userinfo&state=${encodeURIComponent(state)}#wechat_redirect`;

    console.log(`[WeChat Mobile] Generated mobile OAuth URL for next: ${next}`);
    console.log(`[WeChat Mobile] Base URL: ${baseUrl}`);
    console.log(`[WeChat Mobile] Redirect URI: ${decodeURIComponent(redirectUri)}`);
    ...
    console.error('[WeChat Mobile] Error generating mobile OAuth URL:', error);
    return NextResponse.json(
      { error: "Failed to generate WeChat mobile OAuth URL" },
      { status: 500 }
    );
  }
}
