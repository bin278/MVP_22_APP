...
function validateGeneratedCode(code: string): { valid: boolean; errors: string[] } {
  ...
  // 检查5: 检查常见的未定义变量（left, right, top, bottom, store, useSelector, useDispatch 等）
  const commonUndefinedVars = ['\\bright\\b', '\\bleft\\b', '\\btop\\b', '\\bbottom\\b', '\\bstore\\b', '\\buseSelector\\b', '\\buseDispatch\\b']
