...
-- 创建索引以提高查询性能
CREATE INDEX IF NOT EXISTS idx_generation_tasks_taskId ON public.generation_tasks(taskId);
CREATE INDEX IF NOT EXISTS idx_generation_tasks_userId ON public.generation_tasks(userId);
CREATE INDEX IF NOT EXISTS idx_generation_tasks_status ON public.generation_tasks(status);
CREATE INDEX IF NOT EXISTS idx_generation_tasks_createdAt ON public.generation_tasks(createdAt DESC);

-- 启用行级安全性(RLS)
ALTER TABLE public.generation_tasks ENABLE ROW LEVEL SECURITY;

-- 允许所有用户读取任务(用于查询自己的任务)
CREATE POLICY IF NOT EXISTS "Users can view all tasks" ON public.generation_tasks FOR SELECT USING (true);

-- 允许所有用户插入任务(用于创建任务)
CREATE POLICY IF NOT EXISTS "Users can insert tasks" ON public.generation_tasks FOR INSERT WITH CHECK (true);

-- 允许所有用户更新任务(用于更新任务状态)
CREATE POLICY IF NOT EXISTS "Users can update tasks" ON public.generation_tasks FOR UPDATE USING (true);

-- 允许所有用户删除任务(用于取消任务)
CREATE POLICY IF NOT EXISTS "Users can delete tasks" ON public.generation_tasks FOR DELETE USING (true);
